...
//setting a directory entry given an inode number and file name 
...
void write_superblock(int fd) {
	off_t off = lseek(fd, BLOCK_OFFSET(1), SEEK_SET);  //super block starts at 1024, moving file pointer to this location of the open file
	//the file descriptor that we will input is the hardware- floppy disk, etc
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; //root directory and lost and found directory 
...
void write_root_dir_block(int fd)
{
	...
	struct ext2_dir_entry link_entry = {0};
	dir_entry_set(link_entry, HELLO_INO, "hello"); 
	dir_entry_write(link_entry, fd);
	bytes_remaining -= link_entry.rec_len;
...
void write_lost_and_found_dir_block(int fd) {
	...
	//first you seek into the proper offset
	...
	ssize_t bytes_remaining = BLOCK_SIZE;
	//declear, initialize and set the curent entry- have to do this for all the inodes
	struct ext2_dir_entry current_entry = {0};
...
void write_hello_world_file_block(int fd)
{
	...
	u8 hello_world_file_block[BLOCK_SIZE] = {0}; //1024 8 bit values--> total 8172. 
	//write hello world into the data blocks like this
	char *s = "Hello world\n";                                                 
	size_t len = strlen(s);                                        
	memcpy(hello_world_file_block, s, len);                                   

	if (write(fd, hello_world_file_block, BLOCK_SIZE) != BLOCK_SIZE)
	{
		errno_exit("write");
	}
